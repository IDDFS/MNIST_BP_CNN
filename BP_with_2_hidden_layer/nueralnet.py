...
class NueralNetwork(object):
    def __init__(self, input_size, hidden_size1, hidden_size2, output_size, learning_rate = 1, lambda_ = 0.001):
    ...
    def comopute_loss(self, y, y_hat):
        ...
        regularization = (self.lambda_ / (2 * m)) * (np.sum(self.w1**2) + np.sum(self.w2**2) + np.sum(self.w3**2))
    ...
    def backward(self, X, y, y_hat):
        ...
        dw3 = (self.a2.T.dot(error) + self.lambda_ * self.w3) / m 
        db3 = np.sum(error, axis=0) / m
        # 输出层的delta

        delta1 =  error.dot(self.w3.T) * self.sigmoid_derivative(self.z2)
        dw2 = (self.a1.T.dot(delta1) + self.lambda_ * self.w2) / m
        db2 = np.sum(delta1, axis=0) / m
        # 隐藏层2的delta

        delta2 = delta1.dot(self.w2.T) * self.sigmoid_derivative(self.z1)
        dw1 = (X.T.dot(delta2) + self.lambda_ * self.w1) / m
